...
export function BookingWizard() {
  ...
  if (confirmed) return <Card className="panel-glow mx-auto max-w-xl border-white/20 bg-white/[.04]"><CardContent className="flex flex-col items-center p-8 text-center sm:p-12"><div className="grid size-16 place-items-center rounded-full bg-white text-black"><Check className="size-7" /></div><Badge className="mt-6">Reserva confirmada</Badge><h1 className="font-heading mt-4 text-3xl font-semibold">Sua cadeira está reservada.</h1><p className="mt-3 max-w-sm text-sm leading-6 text-muted-foreground">{service.name} · {date} às {time} com {selectedStaff?.name ?? "o primeiro profissional disponível"}.</p><div className="mt-7 w-full rounded-2xl border border-white/10 bg-black/20 p-4 text-left"><div className="flex justify-between text-sm"><span>Total</span><span>€ {service.price}</span></div><div className="mt-2 flex justify-between text-sm text-white"><span>Sinal pago</span><span>− € {deposit}</span></div><Separator className="my-3" /><div className="flex justify-between font-medium"><span>Saldo no local</span><span>€ {service.price - deposit}</span></div></div><div className="mt-7 flex w-full flex-col gap-2 sm:flex-row"><Button asChild className="flex-1"><Link href="/">Ir para minha área</Link></Button><Button asChild variant="outline" className="flex-1"><Link href={`/barbearia/${business.slug}`}>Voltar à barbearia</Link></Button></div></CardContent></Card>;
    ...
    <div className="mx-auto grid max-w-6xl gap-5 lg:grid-cols-[1fr_340px]">
      <Card className="panel-glow border-white/10">
        ...
        <CardContent className="p-5 sm:p-7">
          ...
          {step === 1 ? <div className="grid gap-3 sm:grid-cols-2"><button type="button" onClick={() => setStaffId("any")} className={cn("rounded-2xl border p-4 text-left", staffId === "any" ? "border-primary/50 bg-primary/8" : "border-white/8")}><div className="grid size-10 place-items-center rounded-full bg-white/10 text-white"><UserRound /></div><p className="mt-8 font-medium">Qualquer profissional</p><p className="mt-1 text-xs text-muted-foreground">O primeiro disponível para este horário.</p></button>{staff.map((member) => <button type="button" key={member.id} onClick={() => setStaffId(member.id)} className={cn("rounded-2xl border p-4 text-left", staffId === member.id ? "border-primary/50 bg-primary/8" : "border-white/8")}><Avatar><AvatarFallback className="bg-white/10 text-white">{member.initials}</AvatarFallback></Avatar><p className="mt-8 font-medium">{member.name}</p><p className="mt-1 text-xs text-muted-foreground">{member.role}</p></button>)}</div> : null}
      ...
      <Card className="h-fit border-white/8 lg:sticky lg:top-5"><CardHeader><CardTitle className="font-heading text-base">Resumo da reserva</CardTitle></CardHeader><CardContent className="flex flex-col gap-4"><div><p className="text-xs text-muted-foreground">Serviço</p><p className="mt-1 text-sm font-medium">{service.name}</p><p className="text-xs text-muted-foreground">{service.duration} min</p></div><Separator /><div><p className="text-xs text-muted-foreground">Profissional</p><p className="mt-1 text-sm font-medium">{selectedStaff?.name ?? "Qualquer disponível"}</p></div><div><p className="text-xs text-muted-foreground">Data e horário</p><p className="mt-1 text-sm font-medium">{date} · {time}</p></div><Separator /><div className="flex justify-between text-sm"><span>Total</span><span>€ {service.price}</span></div><div className="flex justify-between text-sm text-muted-foreground"><span>Sinal agora</span><span>€ {deposit}</span></div><div className="flex justify-between font-medium"><span>No local</span><span>€ {service.price - deposit}</span></div><div className="rounded-xl border border-white/10 bg-white/[.04] p-3 text-xs leading-5 text-white"><CalendarCheck className="mr-2 inline size-4" /> Confirmação imediata após o sinal.</div></CardContent></Card>
    </div>
  );
}
