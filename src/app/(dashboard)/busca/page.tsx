...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const [customers, appointments] = query ? await Promise.all([
    db.customer.findMany({ where: { tenantId: session.tenantId, deletedAt: null, ...(restrictToProfessional ? { appointments: { some: { staffId } } } : {}), OR: [{ firstName: { contains: query } }, { lastName: { contains: query } }, { phone: { contains: query } }, { email: { contains: query } }] }, take: 20 }),
    db.appointment.findMany({ where: { tenantId: session.tenantId, deletedAt: null, ...(restrictToProfessional ? { staffId } : {}), OR: [{ customer: { firstName: { contains: query } } }, { customer: { lastName: { contains: query } } }, { services: { some: { service: { name: { contains: query } } } } }] }, include: { customer: true, staff: true, services: { include: { service: true } } }, take: 20 }),
  ]) : [[], []];
