  ...
  return (
    <div className="flex flex-col gap-6">
      <section className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          <div className="mb-2 flex items-center gap-2 text-xs text-muted-foreground"><span className="size-2 rounded-full bg-[#B7F34A] shadow-[0_0_10px_#B7F34A]" /> Operação em tempo real</div>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {dashboardMetrics.map((metric, index) => (
          <Card key={metric.label} className={index === 0 ? "border-primary/25 bg-gradient-to-br from-primary/12 to-card" : "border-white/8"}>
            ...
            <CardContent className="flex items-end justify-between"><p className="font-heading text-3xl font-semibold tracking-[-.04em]">{metric.value}</p><Badge variant={metric.tone === "positive" ? "default" : "secondary"} className={metric.tone === "positive" ? "bg-[#B7F34A]/12 text-[#B7F34A]" : ""}>{metric.change}</Badge></CardContent>
          </Card>
        ))}
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.45fr_.75fr]">
        ...
        <Card className="overflow-hidden border-primary/20 bg-[radial-gradient(circle_at_top_right,rgba(139,92,246,.2),transparent_52%)]">
        ...
        <Card className="border-white/8">
          ...
          <CardContent>
            <div className="relative flex flex-col gap-1 before:absolute before:bottom-4 before:left-[3.45rem] before:top-4 before:w-px before:bg-gradient-to-b before:from-primary before:via-primary/30 before:to-transparent">
              {appointments.slice(0, 5).map((appointment, index) => <div key={appointment.id} className="group relative grid grid-cols-[3rem_1rem_1fr] items-center gap-2 rounded-xl px-2 py-2.5 hover:bg-white/[.025]"><span className="font-mono text-xs text-muted-foreground">{appointment.time}</span><span className={`relative z-10 size-2 rounded-full ${index <= 1 ? "bg-primary shadow-[0_0_12px_#8B5CF6]" : "bg-muted-foreground"}`} /><div className="flex min-w-0 items-center justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-medium">{appointment.customer}</p><p className="truncate text-xs text-muted-foreground">{appointment.service} · {appointment.staff}</p></div><Badge variant={statusVariant(appointment.status)}>{appointment.status}</Badge></div></div>)}
            </div>
          </CardContent>
        </Card>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <Card className="border-white/8"><CardHeader><CardTitle className="font-heading">Insights</CardTitle><CardDescription>Recomendações baseadas no ritmo da operação.</CardDescription></CardHeader><CardContent className="flex flex-col gap-3">{insights.map((insight) => <div key={insight.title} className="flex gap-3 rounded-xl border border-white/8 bg-white/[.018] p-3"><span className={`mt-0.5 grid size-8 shrink-0 place-items-center rounded-lg ${insight.kind === "warning" ? "bg-amber-500/10 text-amber-400" : "bg-primary/10 text-primary"}`}>{insight.kind === "warning" ? <CircleAlert className="size-4" /> : <TrendingUp className="size-4" />}</span><div><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-medium">{insight.title}</p><Badge variant="secondary">{insight.impact}</Badge></div><p className="mt-1 text-xs leading-5 text-muted-foreground">{insight.detail}</p></div></div>)}</CardContent></Card>
      </section>
    </div>
  );
}
